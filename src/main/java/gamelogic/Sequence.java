...
public class Sequence {
    ...
    public boolean isSequenceFailed(GameState gameState) {
        ...
        // TODO: MAKE FUNCTION
        List<String> bufferString = gameState.buffer.stream().map(gameState.gameLevel.matrix::get).collect(Collectors.toList());
        ...
        if (matchIndex < 0) return gameState.buffer.size() + seq.size() > gameState.getCurrentBufferLength();
    ...
    public boolean isSequenceCompleted(GameState gameState) {
        ...
        String bufferString = gameState.buffer.stream().map(cl -> gameState.gameLevel.matrix.get(cl) + " ").reduce("", String::concat);
